...
class IndexController extends H {
    ...
    public function actionIndex() {
        ...
        $question = Question::model()->count('status=' . $status);
        $answer = Answer::model()->count('status=' . $status);
        $poipost = PoiPost::model()->count('status=' . $status);
        $commentsNum = Comments::model()->count('status=' . $status);
        $attachsNum = Attachments::model()->count('status=' . $status);
        $poiErrorNum = PoiError::model()->count('status=' . $status);
        $feedbackNum = Feedback::model()->count('status=' . $status);
        $arr = array(
            'posts' => $posts,
            'question' => $question,
            'answer' => $answer,
